...
![](//Day01-15/Day08/res/object-feature.png)
![](https://code.visualstudio.com/assets/docs/editor/tasks/typescript-build.png)
1. set可以集合操作，自动uniq，sort。
2. tuple不可改，利于共享，节省资源。
3. string 本身就很方便，有众多方法可用。 
4. list 列表，创建方法有意思。 相比**生成式**，**生成器**不占用存储数据的空间。。
5. sample函数：实现从列表中选择不重复的n个元素。
